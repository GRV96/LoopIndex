...
class LoopIndex:
    ...
    def __init__(self, limit, jump=1, start=0):
        ...
        if not self._limit_is_reachable():
            raise ValueError('The limit will never be reached.')

        self.reset()

        if self._jump > 0:
            self._lambda_within_bounds = lambda: self._index + self._jump - 1 < self._limit
        else:
            self._lambda_within_bounds = lambda: self._index + self._jump + 1 >= self._limit

    def __repr__(self):
        return 'LoopIndex(' + str(self._limit) + ', ' + str(self._jump) + ', ' + str(self._start) + ')'
